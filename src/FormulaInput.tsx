...
const FormulaInput = ({state: propState, allowPred}: Props) => {
    ...
    return <>
        <input pattern={allowPred ? finalPatternPred : finalPattern} onKeyPress={handleKeyPress} ref={ref} onChange={handleChange} name={pathToPHPFormName(state.path)} className="bg-gray-100 w-52 h-12" required aria-required value={state.value}/>
    </>
}
